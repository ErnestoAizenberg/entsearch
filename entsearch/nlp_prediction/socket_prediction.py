...
class DatasetDTO:
    ...
    def __repr__(self):
        return (f"DatasetDTO(dataset_name='{self.dataset_name}', "
...
def filter_by_threshold(
    ...
    result = DatasetDTO(
        ...
        descriptions=[descriptions[i] for i in filtered_indices 
...
def handle_prediction(data: dict) -> None:
    ...
    try:
        ...
        for ds_uuid in dataset_uuids:
            ...
            if not probabilities:
                logger.warning(f"Skipping dataset due to probability calculation error: {ds_uuid}")
                continue
