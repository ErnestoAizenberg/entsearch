...
def get_dataset_data(entity_repo, ds_uuid):
    ...
    return DatasetDTO(names=lst_names, descriptions=lst_descriptions, dataset_name=dataset_model.title)
...
def filter_by_threshold(
    ...
    if by_description:
        filtered_descriptions = [descriptions[i] for i in filtered_indices if i < len(descriptions)]
    else:
...
def handle_prediction(data):
    ...
    try:
        ...
        for ds_uuid in dataset_uuids:
            ...
            returning_dataset_dto, filtered_probs, filtered_indices = filter_by_threshold(
                names=dataset_dto.names,
            ...
            logger.info(f'Response data prepared for dataset id {ds_uuid}: {response_data}')
            emit('data_ready', response_data)
            time.sleep(0.1)  # Небольшая задержка между отправками
            
    except Exception as e:
        logger.error(f'Error occurred while processing prediction request: {e}', exc_info=True)
        emit('error', str(e))
